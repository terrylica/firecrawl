...
function startServices(command?: string[]): Services {
  ...
  const nuqWorkers = Array.from(
    { length: Number(process.env.NUQ_WORKER_COUNT ?? 5) },
    (_, i) =>
      execForward(
        `nuq-worker-${i}`,
        process.argv[2] === "--start-docker"
...
async function runDevMode(): Promise<void> {
  ...
  try {
    ...
    // FIXME: this can create a hanging process it doesn't kill them all properly always
